...
import { processDirectives, type DirectiveContext } from '../src/shell/directives.js';
...
describe('processDirectives', () => {
  it('@inertia expands to <div id="app" data-page="..."> in CSR mode', () => {
  ...
  it('@vite(entry) in prod: emits hashed bundle from manifest', () => {
    ...
    const out = processDirectives("@vite('app/client.tsx')", baseCtx({
      isDev: false,
      manifest,
    }));
  ...
  it('@vite(entry) in prod throws when entry not in manifest', () => {
    expect(() => processDirectives("@vite('missing.tsx')", baseCtx({
      isDev: false,
      manifest: {},
  ...
  it('@asset in prod returns hashed path from manifest', () => {
    ...
    const out = processDirectives("@asset('favicon.svg')", baseCtx({
      isDev: false,
      manifest,
    }));
  ...
  it('@asset returns raw path in prod when not in manifest (fallback)', () => {
    const out = processDirectives("@asset('static/logo.png')", baseCtx({
      isDev: false,
      manifest: {},
    }));
