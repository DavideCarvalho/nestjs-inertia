...
describe('CsrfCookieInterceptor', () => {
  it('writes XSRF-TOKEN cookie when not present', async () => {
    ...
    await lastValueFrom(interceptor.intercept(makeCtx(req, res), { handle: () => of(undefined) }), { defaultValue: undefined });
    expect(cookieSet).toHaveBeenCalledWith('XSRF-TOKEN', expect.stringMatching(/[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/), expect.objectContaining({
      httpOnly: false,
      sameSite: 'lax',
    }));
  });
