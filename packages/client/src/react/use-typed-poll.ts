...
/**
 * Typed wrapper over Inertia v2's native `usePoll`.
 *
 ...
 * not TanStack Query).
 *
 * Pass the codegen `route` resolver (the same one given to `<InertiaRouteProvider>`)
 * so the href is resolved type-safely:
 *
 * ```ts
