/**
 * Tests for the native polling / prefetch helpers (Svelte).
 *
 * These wrap Inertia v2's native `usePoll` and `router.prefetch`, adding a type
 * layer keyed by page prop names (`only` / `except`). We mock @inertiajs/svelte
...
const { useTypedPoll, prefetchRoute } = await import('../../src/svelte/use-typed-poll.js');

describe('useTypedPoll (Svelte)', () => {
