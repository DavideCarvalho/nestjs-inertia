...
### Field union as a type

Need the filterable-field union on its own — e.g. to validate dynamic column ids from a data-grid before passing them to the builder? Use the [`Route.FilterFields<K>`](../guides/typed-client) helper instead of reaching into the builder with `Parameters<...>`:
