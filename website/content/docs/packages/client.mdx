...
| `useForm(data)` | Typed wrapper over the official `useForm`; `data`/`errors` keyed by `TForm` ([Forms & Validation](/docs/guides/forms)) |
...
See [Typed Client guide](/docs/guides/typed-client) for the HTTP client, `useTypedPoll`, `prefetchRoute`, `<Deferred>`, and `<WhenVisible>`.

See [Forms & Validation](/docs/guides/forms) for `useForm` and the server-side validation round-trip.
