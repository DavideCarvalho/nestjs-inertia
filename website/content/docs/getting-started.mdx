...
The fastest way to get Inertia.js running in NestJS is two steps: install the packages, then run `nestjs-inertia init` (from `@dudousxd/nestjs-inertia-cli`). The init command scaffolds the config files and prints the exact `app.module.ts` and `main.ts` wiring to paste. Most projects are done in under three minutes.
...
- **Node.js 20+**
- **NestJS 10+** (v10, v11 and v12 are supported)
- **TypeScript 5+**
...
<Tabs items={['pnpm', 'npm', 'yarn']}>
  ...
  <Tab value="npm">
    ...
    npm install --save-dev @dudousxd/nestjs-inertia-cli @dudousxd/nestjs-codegen @dudousxd/nestjs-inertia-codegen-extension @dudousxd/nestjs-inertia-testing
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @dudousxd/nestjs-inertia @dudousxd/nestjs-inertia-vite @dudousxd/nestjs-inertia-client
    yarn add -D @dudousxd/nestjs-inertia-cli @dudousxd/nestjs-codegen @dudousxd/nestjs-inertia-codegen-extension @dudousxd/nestjs-inertia-testing
    ```
  </Tab>
</Tabs>
...
This command detects your frontend framework from `package.json` (pass `--framework react|vue|svelte` to override) and scaffolds the config files in one shot.

### Files created by init

| File | What it is |
|---|---|
| `nestjs-codegen.config.ts` | Shared codegen config (with `nestjsInertiaCodegen()` registered) exported as ONE object -- import it into `NestjsCodegenModule.forRoot` too, so the CLI and module entrypoints never drift |
| `nestjs-inertia.d.ts` | Stub for handwritten registry augmentations (codegen writes the generated ones into `.nestjs-inertia/pages.d.ts`) |
| `inertia/index.html` | HTML shell with the `@inertia` / `@inertiaHead` / `@vite` directives |
| `inertia/app.tsx` (or `.ts`) | Frontend entry point bootstrapping `createInertiaApp` |
| `vite.config.ts` | Vite config with the framework plugin + `nestInertia` plugin + `@`/`~`/`~codegen` aliases |
| `.gitignore` | Created or appended with `.nestjs-inertia/` |

`init` then prints the manual wiring as next steps: the `InertiaModule.forRoot(...)` registration snippet (with `codegen: { enabled: false }`), the `setupInertiaVite` bootstrap for `main.ts`, and the `codegen` / `build:client` scripts to add to `package.json` -- see Step 3 below.

Every create is idempotent: re-running `init` skips files that already exist (pass `--force` to overwrite), and `--dry-run` previews what would be written without touching anything.

<Callout type="info">
  If `vite.config.ts` already exists it is left untouched. Run `nestjs-inertia doctor` afterwards to verify it references the `nestInertia` plugin.
</Callout>

---

## Step 3 -- Wire the module and Vite

`init` prints these exact snippets; paste them into your app.

**`src/app.module.ts`** -- register `InertiaModule` (note `codegen: { enabled: false }` -- the module's legacy auto-watch targets the old codegen package; the `nestjs-codegen` CLI replaces it, see the [Codegen guide](/docs/guides/codegen#auto-watch)):
...
<Callout type="info" title="Why resolve(__dirname)?">
  The compiled code runs from `dist/src/`, not the project root. Using `resolve(__dirname, '..')` ensures the path resolves correctly both locally and in Docker (where only `dist/` is shipped). Add an asset entry for `inertia/` in `nest-cli.json` (`compilerOptions.assets`) so `nest build` copies the shell template into `dist/`.
</Callout>
...
**Done.** Run `nest start --watch` and open [http://localhost:3000](http://localhost:3000). Verify the setup any time with `nestjs-inertia doctor`.
...
<Callout type="info" title="Explicit Zod schemas">
  If you prefer explicit Zod schemas (runtime validation + type extraction), see the [defineContract + @ApplyContract docs](/docs/guides/codegen#priority-chain). Both paths work -- DTOs are zero-config, Zod is explicit.
</Callout>
...
Run `pnpm nestjs-codegen --watch` alongside `nest start --watch` to regenerate types on every controller/page change. `InertiaModule` can also auto-start a watcher on bootstrap in dev mode — see [Auto-watch](/docs/guides/codegen#auto-watch) for the exact conditions and a caveat about the package it currently targets. Either way, codegen is disabled automatically in production.
...
```bash
pnpm exec nestjs-codegen
pnpm exec tsc --noEmit
pnpm run build
```

<Callout type="warn" title="Docker deployments">
  If your Dockerfile only copies `dist/` to the runtime image, the shell template (e.g. `inertia/index.html`) won't be available. Add an asset entry for `inertia/` in `nest-cli.json` so `nest build` copies it into `dist/`, and write `rootView` with `resolve(__dirname, ...)` so the path resolves correctly from the compiled output. Run `nestjs-inertia doctor` to verify your setup.
</Callout>
...
The `init` command prints the `build:client` script to add to `package.json` (add a `build:ssr` variant with `vite build --ssr` if you enable SSR). The full production build sequence:
...
my-nestjs-app/
├── src/
│   ├── main.ts                   ← NestJS bootstrap + setupInertiaVite (snippet printed by init)
│   ├── app.module.ts             ← InertiaModule.forRoot(...) (snippet printed by init)
│   └── home.controller.ts        ← your @Inertia('Home') handler
├── inertia/
│   ├── app.tsx                   ← createInertiaApp entry (created by init)
│   ├── index.html                ← HTML shell with Inertia page script (created by init)
│   └── pages/
│       └── Home.tsx              ← your page components
