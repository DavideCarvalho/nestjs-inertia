...
`nestjsInertiaCodegen(options?)` always contributes the `navigate()` helper to `api.ts`. Two **opt-in, additive** options make it emit extra files — called with no arguments, it emits nothing beyond that helper:

```ts
extensions: [
  nestjsInertiaCodegen({
    // Emit shared.ts — the `InertiaSharedProps` type
    shared: { module: './src/shared-props.ts', export: 'getSharedProps', kind: 'function' },
    // Emit page-excludes.ts — every @Inertia page route (path + method)
    pageExcludes: true,
  }),
],
```

### `shared` — emit `shared.ts`

Emits an `InertiaSharedProps` type derived from your shared-props source, so the client can type `usePage().props` against it. `SharedPropsSource` is:

| Field | Type | Description |
|-------|------|-------------|
| `module` | `string` | Path to the module exporting the shared-props shape, relative to `cwd` |
| `export` | `string` | The named export to read |
| `kind` | `'function' \| 'type'` | `'function'`: the export is a factory (e.g. the `share()` callback) and its **return type** is used. `'type'`: the export **is** the shape (a `type` alias or `interface`). |

```ts
// kind: 'function' — reads ReturnType of the share factory
shared: { module: './src/shared-props.ts', export: 'getSharedProps', kind: 'function' }
// kind: 'type' — the export is already the shape
shared: { module: './src/types.ts', export: 'SharedProps', kind: 'type' }
```

### `pageExcludes: true` — emit `page-excludes.ts`

Emits every `@Inertia` page route (its path and HTTP method) as an array ready to feed [`setGlobalPrefix`](/docs/recipes/global-prefix)'s `exclude`, so your API prefix skips the Inertia page routes:

```ts
import { pageExcludes } from './.nestjs-inertia/page-excludes.js';

app.setGlobalPrefix('api', { exclude: pageExcludes });
```

<Callout type="warn">
`pageExcludes: true` **throws** if code generation discovers zero `@Inertia` pages — a fail-fast guard against silently emitting an empty exclude list (which would route your pages under `/api`). It resolves each decorator to its import origin via AST inspection, so re-exports and `import * as X` namespaces are handled correctly.
</Callout>
...
`InertiaModule` can start a codegen file watcher automatically on bootstrap in dev mode, so `nest start --watch` alone regenerates files on every controller or page save — no extra command, no extra terminal.
...
| `@dudousxd/nestjs-inertia-codegen` | resolvable (dynamically imported; peer-optional) |
| `nestjs-inertia.config.ts` | loadable at project root |
...
<Callout type="warn">
  This auto-watch integration predates the `@dudousxd/nestjs-codegen` extraction and still targets the legacy `@dudousxd/nestjs-inertia-codegen` package name, not `@dudousxd/nestjs-codegen`. If you only installed `@dudousxd/nestjs-codegen` (the current, supported codegen — see [Install](#install) above), the auto-watch import will fail to resolve and log a warning; it does not block boot. Run the CLI watcher explicitly instead — see [CLI watch mode](#cli-watch-mode) below — or set `codegen: { enabled: false }` to silence the warning.
</Callout>
...
The `nestjs-inertia doctor` command (from [`@dudousxd/nestjs-inertia-cli`](/docs/getting-started)) diagnoses the known setup pitfalls: `InertiaModule` registration, the legacy auto-watch left enabled (`codegen: { enabled: false }` missing), CLI/module codegen config drift, root view shell + Vite entry + `nestInertia` plugin, the Vite client manifest after a build, `@dudousxd/nestjs-inertia >= 1.4.4` (streaming fix), the flat registry `.d.ts` shape, peer version coherence, and `.gitignore`.

```bash
pnpm exec nestjs-inertia doctor
```

Each check prints pass/warn/fail with a one-line fix pointer to the relevant docs page; the command exits `1` when any check fails.
