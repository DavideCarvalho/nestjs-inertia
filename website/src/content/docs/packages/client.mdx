...
| Export | Description |
|--------|-------------|
| `@As(name)` | Set the route name for a controller class or method (read by codegen) |
...
| `Route.Response<K>` / `Route.Body<K>` / `Route.Query<K>` | Extract request/response types by route name |
