...
|---|---|
| `@inertia` | `<div id="app"></div>` plus `<script data-page="app" type="application/json">{page data}</script>`. The page JSON is XSS-safe escaped. |
...
Response types are inferred automatically from the controller method via `ReturnType<import(...)>` — no explicit return type annotations needed.
...
These types are derived from your controller method signatures via `ReturnType<import(...)>` — they stay in sync automatically without explicit type annotations.
...
This lets you write:
- `import { api } from '~codegen/api'` instead of relative paths
- `import { SomeService } from '@/some/service'` for backend imports
- `import { MyComponent } from '~/components/MyComponent'` for frontend imports
