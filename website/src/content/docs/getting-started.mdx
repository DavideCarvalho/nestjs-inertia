...
description: Add Inertia.js to an existing NestJS project in minutes — install, run nestjs-inertia init, wire two lines, and you're done.
---

import { Tabs, TabItem, Aside, Steps } from '@astrojs/starlight/components';

The fastest way to get Inertia.js running in NestJS is three steps: install the packages, run `nestjs-inertia init`, then wire two lines into your existing `app.module.ts` and `main.ts`. Most projects are done in under five minutes.
...
| `views/shell.html` (or `.hbs`/`.ejs`/`.pug`) | HTML shell with `@inertia`, `@inertiaHead`, `@vite` directives |
...
It also patches `.gitignore` with `.nestjs-inertia/`, adds `build:client` and `build:ssr` scripts to `package.json`, and installs any missing framework dependencies.
...
### Step 3 — Wire two lines into your app

`init` creates all the files but deliberately does not modify your existing `app.module.ts` or `main.ts` — auto-patching those files is too risky. You add two things manually.

**3a. Register `InertiaModule` in your AppModule:**
...
├── views/
│   └── shell.html                ← HTML shell with @inertia directives (created by init)
