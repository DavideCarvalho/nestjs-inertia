...
Inject auth headers into every fetcher request. The callback runs per-request so tokens are always fresh.

**React — `headers` prop on `InertiaRouteProvider`:**
...
// In your root layout:
<InertiaRouteProvider routes={route} headers={headers}>
  <App />
</InertiaRouteProvider>
```

**Vue / Svelte / vanilla JS — `setGlobalHeaders`:**

```ts
import { setGlobalHeaders } from '@dudousxd/nestjs-inertia-client';
