...
```ts
import type { RouteParamsMap } from '~codegen/routes';
...
The route name is derived automatically from the controller. No `defineContract` or `@ApplyContract` needed:

```ts
// The codegen reads this and derives route name 'users.show'
@Controller('/users')
