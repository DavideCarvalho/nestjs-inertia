...
The route name passed to `<Link route="..." />` is **auto-derived** from `<ControllerClass>.<method>` by stripping the `Controller` suffix and lowercasing the first letter. For example, `UsersController.show` → `users.show`.
...
// In the controller — name is derived from UsersController.show → 'users.show'
@Get('/users/:id')
...
Use `@As('custom.name')` to override the auto-derived name when needed:
...
@Get('/users/:id')
@ApplyContract(ShowUser)
@As('user.profile')   // overrides auto-derived 'users.show'
show() { ... }
```
