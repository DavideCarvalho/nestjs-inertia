...
const ShowUser = Contract.get('/users/:id', {
...
```tsx
// Frontend usage — name comes directly from Contract.name
<Link route="users.show" routeParams={{ id: user.id }}>View profile</Link>
```

**Best practice:** keep `Contract.name` aligned with your resource hierarchy so that `route('users.show')` and `api.users.show` both point to the same endpoint — one typed URL builder, one typed data fetcher.
