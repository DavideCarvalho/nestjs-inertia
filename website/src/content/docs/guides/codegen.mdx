...
The API name is **derived automatically** from `<ControllerClass>.<method>` by stripping the `Controller` suffix and lowercasing the first letter.

| Controller class | Method | Derived name |
|------------------|--------|--------------|
| `UsersController` | `list` | `users.list` |
| `UsersController` | `show` | `users.show` |
| `AdminUsersController` | `create` | `adminUsers.create` |

Use dot-notation in the derived name to produce a nested path under `api.*.*`:

| Derived name       | Generated accessor            |
|--------------------|-------------------------------|
...
When the auto-derived name isn't what you want, use `@As('custom.name')` at the method level:
...
class CrewController {
  ...
  @As('crew.directory.fetch')   // overrides auto-derived 'crew.list'
  list() { ... }
}
```
